...
#FOR A GIVEN DATE CHANGE THIS
def get_tweet_counts(query, datetimes):
    tweet_counts = {}
    client = tweepy.Client(bearer_token=config.BEARER_TOKEN)
    for date in datetimes:
        for i in range(0,25):
...
def get_stock_and_tweet_activity(tweet_counts, stock_counts):
    data = {}
    for time in tweet_counts:
        data[time] = (tweet_counts[time], stock_counts[time])
    return data

d = get_stock_and_tweet_activity(tweet_counts, netflix_stock)
print(d)
